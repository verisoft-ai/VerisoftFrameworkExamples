...
public class MultipleDriversClassTest{

    @Autowired
    @DriverUrl("grid")
    URL path;
    //  URL path = new URL("http://95.216.217.133:4444/");



    @Test
    public void twoDriversWithBeanCapabilitiesAndOneOfThemWithURL(
        @DriverName("myChromeDriver") @DriverCapabilities("chrome")  VerisoftDriver chromeDriver,
        @DriverName("myEdgeDriver") @DriverCapabilities("edge") VerisoftDriver edgeDriver) {
        chromeDriver.get("http://95.216.217.133:3001/login?nt=1");
        edgeDriver.get("http://95.216.217.133:3001/login?nt=1");
        WebDriver extractedDriver =VerisoftDriverManager.getDriver("myChromeDriver");
        assertTrue(chromeDriver.getTitle().equals(edgeDriver.getTitle()));
        Waits.milliseconds(2000);
    }
